...
const WorkerRatingsAndReviews: React.FC<WorkerRatingsAndReviewsProps> = ({ ratings, reviews }) => {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6">
      ...
      <div className="flex items-center mb-6">
        <div className="flex items-center" aria-label={`Average rating: ${averageRating.toFixed(1)} out of 5 stars`}>
          {[1, 2, 3, 4, 5].map((star) => (
        ...
        {reviews.map((review, index) => (
          <div key={index} className="border-b border-gray-200 pb-4 last:border-b-0">
            <div className="flex items-center mb-2">
              ...
              <span className="ml-2 font-semibold text-sm">{review.userName || `User ${review.userId._id.substr(-4)}`}</span>
            </div>
            <p className="text-gray-600">{review.review}</p>
            <div className="mt-2 text-xs text-gray-400">Booking ID: {review.bookingId.substr(-6)}</div>
          </div>
        ))}
      </div>
